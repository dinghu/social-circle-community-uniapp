<template>
	<view class="container">
		...
		<block v-else>
			...
			<u-input v-model="value" class="input" type="text"></u-input>
		</block>
	...
	export default {
		data() {
			return {
				...
				type: "",
				value: "",
				tagList: ["圈子达人", "旅行", "热爱生活", "摩羯座", "摄影", "读书", "运动", "看电影", "听歌", "美食"],
				addValue: ""
			}
		},
		onLoad(options) {
		...
		methods: {
			...
			submit() {
				...
				let type = this.type
				if (this.type == 'tags') {
					obj[type] = this.value.join(',')
				} else {
					obj[type] = this.value
				}


				this.$H.post("app/user/update", obj).then(res => {
