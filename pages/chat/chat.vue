<template>
	<view>
		<view class="content" @touchstart="hideDrawer">
			<scroll-view class="msg-list" scroll-y="true" :scroll-with-animation="scrollAnimation"
				...
				<view class="row" v-for="(row,index) in msgList" :key="index" :id="'msg'+row.id">
					...
					<block>
						<!-- 自己发出的消息 -->
						<view class="my" v-if="row.fromId == userInfo.userId">
							<!-- 左-消息 -->
							<view class="left">
								<!-- 文字消息 -->
		...
		<view class="input-box" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<!-- #endif -->
			<!-- #ifdef H5 -->
			<view class="more" @tap="showMore" v-if="false">
			...
			<view class="textbox">
				...
				<view class="text-mode" :class="isVoice?'hidden':''">
					<view class="box">
						<textarea auto-height="true" v-model="mTxt" @focus="textareaFocus" placeholder="请输入内容" />
					</view>
	...
	export default {
		...
		methods: {
			initSocket() {
				...
				let url = this.$c.domain + "app/webSocket/chat/" + this.conversationId + "/" +
					this.userInfo.userId
			...
			// 更新阅读状态
			updateChatStatus() {
				this.$H.get("app/message/private/letter/updateChatStatus", {
			...
			loadHistory(e) {
				...
				this.$H.get("app/message/private/letter/chat/" + this.conversationId, {
					...
					for (let i = 0; i < list.length; i++) {
						if (list[i].type && list[i].type == 'user' && list[i].msg.type == "img") {
							list[i].msg.content = this.setPicSize(list[i].msg.content);
							this.msgImgList.unshift(list[i].msg.content.url);
						}
					}
					Viewid = list[list.length - 1].id;
					setTimeout(() => {
			...
			sendText() {
				...
				let content = this.replaceEmoji(this.mTxt);
