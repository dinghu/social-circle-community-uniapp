					...
					<block v-for="(item, index) in topicList" :key="index">
						<navigator :url="'/pages/topic/detail?id=' + item.id">
							<view class="topic-item">
								<u-image width="100rpx" height="100rpx" shape="circle" :src="item.image">
								</u-image>
								<view class="right">
									<view>{{ item.name }}</view>
									<view class="desc">{{ item.content.substring(0, 10) }}</view>
									<view class="num">
										<text>{{ item.joinNum || 0}}人已加入</text>
										<text>{{ item.postNum || 0}}篇内容</text>
									</view>
								</view>
							</view>
						</navigator>
...
<style lang="scss" scoped>
	...
	.u-tab-item-active::before {
		content: "";
		position: absolute;
		border-left: 4px solid $u-type-primary-dark;
		height: 32rpx;
		left: 0;
		top: 39rpx;
	}
