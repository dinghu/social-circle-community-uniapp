<template>
	<view>
		...
		<block v-for="(item,index) in msgList" :key="index">
			<view class="comment-msg-item" @click="updateMessageStatus(item)">
				...
				<view class="msg-c u-m-l-16">
					<view class="msg-c-t">
						<view class="u-m-b-20 u-flex">
							...
							<text class="u-font-24" v-else-if="item.msgType == 4">回复了</text>
						</view>
					...
					<view v-if="item.msgType == 1" @click="goPost(item,item.messagePayLoad.postId)">
						<view class="post-c">
							<view class="post-c-txt">{{item.content}}</view>
						</view>
					</view>
					<view @click="goPostCommenReply(item)" v-else-if="item.msgType == 4">
						<view class="post-c">
							<view class="post-c-txt">{{item.content}}</view>
						</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			goPost(item, postId) {
				this.$H.get('app/post/info/' + postId).then(res => {
					this.$appUtils.goPost(res.data);
				});
				// this.updateMessageStatus(item)
			},
			goPostCommenReply(item) {
				let url = '/pages/post/comment-more?commentId=' + item.messagePayLoad.commentId
				uni.navigateTo({
					url: url
				})
				// this.updateMessageStatus(item)
