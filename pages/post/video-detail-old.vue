<template>
	<view>
		<video class="video_" :enable-danmu="true" :danmu-btn="true" :autoplay="true" :enable-play-gesture="true"
			:controls="false" :show-fullscreen-btn="false" :loop="true" :src="postDetail.videoUrl"></video>
		<view class="tool-box">
			<navigator hover-class="none" :url="'/pages/user/home?uid=' + postDetail.createUid" class="item">
				<u-avatar :src="postDetail.createUserAvatar"></u-avatar>
			</navigator>
			<view class="item">
				<image v-if="postDetail.isCollection == 1" @click="cancelCollection" src="/static/fav-1.png"></image>
				<image v-else @click="addCollection" src="/static/fav.png"></image>
		...
		<view class="footer">
			<navigator :url="'/pages/user/home?uid=' + postDetail.uid" hover-class="none" class="username">
				@{{ postDetail.createUserName }}</navigator>
		...
		<q-popup v-model="commentPopup" height="50vh">
			<scroll-view scroll-y style="height: 45vh;" @scrolltolower="reachBottom">
				<view class="comment-box">
					<view class="title">评论（{{ postDetail.commentNum }}）</view>
					<!-- 帖子内容 -->
					<view class="comment-item" @longpress="onCopy">
						<image @click="jumpUser(postDetail.createUid)" class="avatar"
							:src="postDetail.createUserAvatar"></image>
						<view class="c-content">
							<view class="user">
								<text>{{ postDetail.createUserName }}</text>
							</view>
							<text class="c-txt">{{ postDetail.content }}</text>
							<text class="time">{{ postDetail.createTime | timeFrom }}</text>
						</view>
					</view>
					<view style="margin-bottom: 40rpx;" v-for="(item, index) in commentList" :key="index">
						...
						<view class="comment-item" @longpress="delComment(item, index)">
							<image @click="jumpUser(item.userId)" class="avatar" :src="item.userAvatar">
							</image>
						...
						<view @click="onReply(item2, index, index2)" v-if="item.children.length > 0"
							...
							<view class="user">
								<u-avatar class="avatar" :size="40" :src="item2.userAvatar"></u-avatar>
								<view class="u-head">
...
<script>
	export default {
		...
		methods: {
			...
			getPostDetail() {
				this.$H.get('app/post/info/' + this.postId)
					.then(res => {
